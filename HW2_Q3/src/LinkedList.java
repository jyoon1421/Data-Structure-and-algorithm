...
public class LinkedList <E> {
	...
	private void quickSortRecursive(Node<E> head, Node<E> tail) {
		...
		if (head == null || tail == null || head == tail) return; // 종료 조건을 정렬할게 1개뿐일때로
		
		int pivot = (int)tail.data; // tail을 pivot으로 사용

		int orgLeftNodeIndex = indexOf(head);
		int orgRightNodeIndex = indexOf(tail); // 제일 왼쪽 오른쪽 노드의 위치 인덱스 기억해둠

		Node<E> orgLeftNode = head; // 피봇 제외 가장 왼쪽 노드 저장해둠
		Node<E> orgRightNode = tail.prev; // 피봇 제외 가장 오른쪽 노드 저장해둠

		Node<E> leftNode = head;
		Node<E> rightNode = tail.prev; 	// 정렬을 위해 변수 저장
